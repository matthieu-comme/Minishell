...
int is_builtin(const processus_t* cmd) {
    if (!cmd || !cmd->argv[0]) return 0;  // On enlève la vérification de cmd->argv
 ...
 */
 int builtin_cd(processus_t* cmd) {
    ...
    if (!path) { dprintf(cmd->stderr_fd, "cd: HOME non défini\n"); return -1; }
    if (chdir(path) != 0) { dprintf(cmd->stderr_fd, "cd: impossible d'accéder à %s\n", path); return -1; }
    return 0;
}
 ...
 */
 int builtin_exit(processus_t* cmd) {
    ...
    int code = 0;
    if (cmd->argv[1]) {
        char* end = NULL; long v = strtol(cmd->argv[1], &end, 10);
        if (!end || *end!='\0') { dprintf(cmd->stderr_fd, "exit: argument non numérique\n"); return -1; }
        code = (int)(v & 0xFF);
    }
    exit(code);
}
 ...
 */
 int builtin_export(processus_t* cmd) {
    ...
    for (int i=1; cmd->argv[i]; ++i) {
        ...
        if (!eq) { dprintf(cmd->stderr_fd, "export: format VAR=val requis\n"); return -1; }
        *eq='\0'; char* var = cmd->argv[i]; char* val = eq+1;
        if (setenv(var, val, 1) != 0) { dprintf(cmd->stderr_fd, "export: échec pour %s\n", var); return -1; }
    }
    return 0;
}
 ...
 */
 int builtin_pwd(processus_t* cmd) {
    ...
    char buf[4096];
    if (!getcwd(buf, sizeof buf)) { dprintf(cmd->stderr_fd, "pwd: erreur getcwd\n"); return -1; }
